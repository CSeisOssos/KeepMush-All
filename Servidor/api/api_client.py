...
class NaturalistAPIClient:
    ...
    def buscar_dados_especie(self, scientific_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Extração segura de ranks do iNaturalist
            
            ancestors = inat_data.get("ancestors", [])
            ancestry_lookup = {a.get("rank"): a.get("name") for a in inat_data.get("ancestors", [])}

            taxonomy = {
                "reino": gbif_data.get("kingdom") or ancestry_lookup.get("kingdom") or inat_data.get("iconic_taxon_name", "Desconhecido"),
                "filo": gbif_data.get("phylum") or ancestry_lookup.get("phylum", "Desconhecido"),
                "classe": gbif_data.get("class") or ancestry_lookup.get("class", "Desconhecido"),
                "ordem": gbif_data.get("order") or ancestry_lookup.get("order", "Desconhecido"),
                "familia": gbif_data.get("family") or ancestry_lookup.get("family", "Desconhecido"),
